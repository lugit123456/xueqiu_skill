...
**触发条件**：回执 `diagnosis.need_user_login == true`（雪球 API 返回 `error_code: 10022` 等"请登录"信号）。

**agent 行为**：

1. **不要把命令丢给用户**——直接用你手头的 bash 工具执行：
   ```bash
   cd /Users/luzhe/Desktop/code/agent_skills/xueqiu_skill
   python3 xueqiu_random_trigger.py --debug
   ```
2. 这个命令会**弹出有头 Chromium 浏览器**，引导用户到雪球登录页。等用户在浏览器里完成手机号 + 滑块验证后，subprocess 自然退出。
3. 登录完成后，**自动重新调用** `add_blogger_from_user_input(original_user_text)` 重试抓取。
4. **最多重试 1 次**，避免死循环。
5. 如果重试后仍 `need_user_login`（极少见，通常是 SSH / CI 无 GUI 环境），告诉用户手动跑命令。

**为什么这样设计**：爬虫代码只负责"检测 + 诊断"（输出 `need_user_login=true` 信号），"自动登录"行为由 agent 在 SKILL.md 协议驱动下用 bash 工具执行。这样代码更瘦、agent 灵活性更高（agent 可以根据当前环境决定如何处理，比如检查 GUI 可用性、切换登录方式等）。

**agent 模板回复**（重试仍失败时）：

```text
🔐 自动登录失败（请检查终端是否有弹浏览器弹出来）。
   如果没有 GUI，请手动运行：
   `python3 xueqiu_random_trigger.py --debug`
   完成后，**重新发送刚才的链接/关键词**给我。
```

## Cookie 持久化机制（已自动处理）

**不要被 `context.cookies()` 返回的 `expires: -1` 误导**——雪球的登录 cookie 实际上**已经**被持久化了。

### 机制说明

- `chromium.launch_persistent_context(user_data_dir=...)` 模式下，Chromium 自动把服务端的 `Set-Cookie`（无 `Max-Age/Expires`）"提升"为 ~400 年后过期，并写入 `user_data_dir/Default/Cookies` SQLite。
- 实测 SQLite 里的 `expires_utc` 指向 **2395 年**，`is_persistent=1`。
- 下次启动从 SQLite 加载，**不**会被当作 session cookie 忽略。
- 实战证据：用户 mini 抓取首页 21 条成功入库；所有登录 cookie 在 SQLite 里均带 2395 年 expires。

### 为什么 `context.cookies()` API 返回 `expires: -1`

Playwright `context.cookies()` 返回的是浏览器进程内的**实时内存视图**，对 `Set-Cookie` 没带 `Max-Age/Expires` 的 cookie 报 -1。Playwright 没有把 SQLite 里的 expires 字段同步到内存视图。这只是 **API 表现层的小坑**，**不影响 cookie 实际可用性**。

### 何时会真正失效

- 雪球服务端主动 invalidate token（罕见，需要重新登录）
- `xq_id_token` 的 JWT `exp` 字段到期（**服务端**的 token 过期，cookie 持久化也救不了）

### agent 何时应该触发"重新登录"协议

- 回执中 `diagnosis.need_user_login == true`（即 fetch 返回"请登录"信号）→ 按"登录态失效协议"提示用户跑 `--debug`
- **不要**因为 `diagnosis.cookies` 里有 `expires=-1` 就误判需要重新登录
